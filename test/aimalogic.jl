using FastWFOMC.aimalogic
import FastWFOMC.aimalogic: wumpus_world_inference, horn_clauses_kb, test_fol_kb, crime_kb
...
@test proposition_symbols(expr("x & y & z | A")) == Set([Expression("A")]);

@test proposition_symbols(expr("(x & B(z)) ==> Farmer(y) | A")) == Set([Expression("A"), expr("Farmer(y)"), expr("B(z)")]);
...
@test pl_fc_entails(horn_clauses_kb, Expression("Q")) == true;

@test pl_fc_entails(horn_clauses_kb, Expression("SomethingSilly")) == false;
...
function fol_bc_ask_query(q::Expression; kb::Union{Nothing,AbstractKnowledgeBase} = nothing)
    ...
    if (typeof(kb) <: Nothing)
        answers = fol_bc_ask(test_fol_kb, q)
    else
...
@test fol_bc_ask_query(expr("Criminal(x)"), kb = crime_kb) == [Dict([Pair(Expression("x"), Expression("West"))])];
